...
def ID_upload_positive(customerid):
    ...
    data1 = {"HEAD":common_public.App_head(),"BODY":{"id":customerid,"bizType":"idImg","imageType":'1'}}
    data = json.dumps(data1)
    # 上传文件单独构造成以下形式
    # 'img' 上传文件的键名
...
def identity_check(customerid,frontpath,backpath,holdpath,token):
    ...
    data1 = {
        'HEAD':head,
        'BODY':{"realName":common_public.customer_name(),
...
def set_customer_examine(mobile):
    ...
    mydb =commonORBD.MyORDB()
    mydb.executeSQL('UPDATE TPOS.CIF_CUSTOMER SET EXAMINE_STATE = 1 WHERE MOBILE =%s' % mobile)
    mydb.closeDB()
